  ...
  return (
    <div>
      <h2 id="absence-heading" data-cy="AbsenceHeading">
        <Translate contentKey="jhipstermogodbApp.absence.home.title">Absences</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipstermogodbApp.absence.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/absence/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipstermogodbApp.absence.home.createLabel">Create new Absence</Translate>
          </Link>
        </div>
      </h2>
        ...
        {absenceList && absenceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipstermogodbApp.absence.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('startDate')}>
                  <Translate contentKey="jhipstermogodbApp.absence.startDate">Start Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('startDate')} />
                </th>
                <th className="hand" onClick={sort('endDate')}>
                  <Translate contentKey="jhipstermogodbApp.absence.endDate">End Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('endDate')} />
                </th>
                <th className="hand" onClick={sort('numberDayAbsence')}>
                  <Translate contentKey="jhipstermogodbApp.absence.numberDayAbsence">Number Day Absence</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('numberDayAbsence')} />
                </th>
                <th className="hand" onClick={sort('typeAbsence')}>
                  <Translate contentKey="jhipstermogodbApp.absence.typeAbsence">Type Absence</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('typeAbsence')} />
                </th>
                <th className="hand" onClick={sort('confirmationAbsence')}>
                  <Translate contentKey="jhipstermogodbApp.absence.confirmationAbsence">Confirmation Absence</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('confirmationAbsence')} />
                </th>
                <th className="hand" onClick={sort('congeRestant')}>
                  <Translate contentKey="jhipstermogodbApp.absence.congeRestant">Conge Restant</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('congeRestant')} />
                </th>
                <th>
                  <Translate contentKey="jhipstermogodbApp.absence.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {absenceList.map((absence, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`jhipstermogodbApp.TypeAbsence.${absence.typeAbsence}`} />
                  </td>
                  <td>
                    <Translate contentKey={`jhipstermogodbApp.ConfirmationAbsence.${absence.confirmationAbsence}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipstermogodbApp.absence.home.notFound">No Absences found</Translate>
            </div>
          )
        )}
      </div>
