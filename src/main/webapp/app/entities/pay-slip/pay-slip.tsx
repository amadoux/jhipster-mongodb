  ...
  return (
    <div>
      <h2 id="pay-slip-heading" data-cy="PaySlipHeading">
        <Translate contentKey="jhipstermogodbApp.paySlip.home.title">Pay Slips</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipstermogodbApp.paySlip.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/pay-slip/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipstermogodbApp.paySlip.home.createLabel">Create new Pay Slip</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paySlipList && paySlipList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipstermogodbApp.paySlip.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('netSalaryPay')}>
                  <Translate contentKey="jhipstermogodbApp.paySlip.netSalaryPay">Net Salary Pay</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('netSalaryPay')} />
                </th>
                <th className="hand" onClick={sort('paySlipDate')}>
                  <Translate contentKey="jhipstermogodbApp.paySlip.paySlipDate">Pay Slip Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paySlipDate')} />
                </th>
                <th className="hand" onClick={sort('uploadPaySlip')}>
                  <Translate contentKey="jhipstermogodbApp.paySlip.uploadPaySlip">Upload Pay Slip</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('uploadPaySlip')} />
                </th>
                <th>
                  <Translate contentKey="jhipstermogodbApp.paySlip.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipstermogodbApp.paySlip.home.notFound">No Pay Slips found</Translate>
            </div>
          )
        )}
      </div>
